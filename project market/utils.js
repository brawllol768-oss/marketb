...
function getUserRating(un) { const u = state.users.find(x => x.username === un); if (!u || !u.reviews || !u.reviews.length) return { avg: 0, count: 0 }; const s = u.reviews.reduce((a, r) => a + r.rating, 0); return { avg: s / u.reviews.length, count: u.reviews.length }; }
...
function applyTheme(th) { if (th === 'default') document.documentElement.removeAttribute('data-theme'); else document.documentElement.setAttribute('data-theme', th); localStorage.setItem('bb_theme', th); toast('🎨 Тема изменена', 'success'); }
