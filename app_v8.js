...
window.addEventListener('load', () => {
  ...
  let st = (function(){ try { return localStorage.getItem('bb_theme'); } catch(e) { return null; } })();
  if (st !== 'black' && st !== 'light' && st !== 'default') st = 'default';
  try { if (!localStorage.getItem('cyber_theme_applied')) { st = 'default'; localStorage.setItem('cyber_theme_applied', 'true'); } } catch(e) {}
  applyTheme(st);
...
function fillServers() {
  ...
  let fsHTML = '<option value="all">Все сервера</option>'; 
  let fmHTML = '<option value="">Выберите сервер</option>';
  for (let i = 1; i <= 33; i++) { const o = `<option value="${i}">Сервер ${i}</option>`; fsHTML += o; fmHTML += o; }
  fs.innerHTML = fsHTML; fm.innerHTML = fmHTML;
}
...
document.addEventListener('click', function (e) {
  const btn = e.target.closest('button[data-close], .modal-close');
  if (btn) {
      e.preventDefault();
      e.stopPropagation();
      ...
      // Specifically wipe out the create form just in case
      if (btn.closest('#createModal')) {
         const form = document.getElementById('createForm');
         if (form) form.reset();
         uploadedFiles = [];
         if (typeof filePreview !== 'undefined' && filePreview) filePreview.innerHTML = '';
...
function finalizeProfileUpdate(username, iac, form, d, submitBtn, oh) {
  ...
  state.currentView = 'market'; document.querySelectorAll('[data-view]').forEach(x => x.classList.remove('active')); const mm = document.querySelector('[data-view="market"]'); if (mm) mm.classList.add('active');
  renderView();
...
function handleCreateSubmit(form) {
  ...
  (async () => {
    try {
      ...
      console.error(er); 
      if (er.message && er.message.includes('permission_denied')) {
         toast('❌ Ошибка: Файлы слишком тяжелые для текстовой базы. Уменьшите фото.', 'error');
      } else {
         toast('❌ Ошибка при создании (возможно фото слишком тяжелые)', 'error'); 
      }
    }
...
function handleReviewSubmit(form) {
  const d = Object.fromEntries(new FormData(form));
  if (!state.user) { toast('👤 Сначала создайте профиль', 'warning'); return; }
  const tu = getUserByUsername(d.reviewTarget); if (!tu) { toast('❌ Пользователь не найден', 'error'); return; }
...
function handleActionSubmit(form) {
  ...
  const u = getUserByUsername(fd.targetUsername); if (!u) { toast('❌ Пользователь не найден', 'error'); return; }
  const up = {};
...
function setupFileUpload() {
  ...
  fileInput.addEventListener('change', async (e) => {
    ...
    if (submitBtn) { submitBtn.disabled = true; submitBtn.innerHTML = '⏳ Загрузка фото...'; }
    
    for (const f of fa) { 
      const v = validateFile(f); 
      if (!v.valid) { toast(`⚠️ ${f.name}: ${v.error}`, 'warning'); continue; } 
      toast(`⏳ Сжатие и загрузка ${f.name}...`, 'info');
      try { 
        if (f.type.startsWith('image/')) { 
          const u = await uploadImageToStorage(f); 
          if (u.startsWith('data:') && u.length > 3000000) { toast(`❌ Файл ${f.name} слишком велик для базы`, 'error'); continue; }
          uploadedFiles.push({ type: 'image', url: u, name: f.name, size: f.size }); 
        } else if (f.type.startsWith('video/')) { 
          const u = await uploadVideoToStorage(f); 
          if (u.startsWith('data:') && u.length > 4000000) { toast(`❌ Видео ${f.name} слишком большое`, 'error'); continue; }
          uploadedFiles.push({ type: 'video', url: u, name: f.name, size: f.size }); 
        } 
      } catch (er) { 
    ...
    renderFilePreview(); 
    if (uploadedFiles.length) toast(`✅ Готово: ${uploadedFiles.length} файлов`, 'success');
    if (submitBtn) { submitBtn.disabled = false; submitBtn.innerHTML = '✅ Создать'; }
...
function updateOnlineCounter() {
  ...
  const base = Math.max(15, Math.floor(total * 0.4));
  
  if (currentOnlineCount === 0) {
    currentOnlineCount = base + Math.floor(Math.random() * 5);
  } else {
    // Only 20% chance to change by 1 person
    if (Math.random() > 0.8) {
  ...
  if (currentOnlineCount < base - 2) currentOnlineCount = base - 2;
  if (currentOnlineCount > base + 8) currentOnlineCount = base + 8;
  
  el.textContent = currentOnlineCount + ' онлайн';
}
// Update slowly to not look glitchy
setInterval(updateOnlineCounter, 12000);
setTimeout(updateOnlineCounter, 1000);
